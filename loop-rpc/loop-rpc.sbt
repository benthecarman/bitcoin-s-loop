...
name := "bitcoin-s-loop-rpc"

libraryDependencies ++= Deps.loopRpc
...
Compile / scalacOptions ++= Seq(
  "-Wconf:cat=deprecation:site=looprpc\\..*:silent",
  "-Wconf:cat=unused-imports:site=looprpc:silent"
)
...
TaskKeys.downloadLoop := {
  val logger = streams.value.log
  import scala.sys.process._

  val binaryDir = CommonSettings.binariesPath.resolve("loop")

  if (Files.notExists(binaryDir)) {
    logger.info(s"Creating directory for loop binaries: $binaryDir")
    Files.createDirectories(binaryDir)
  }
  ...
  logger.debug(s"(Maybe) downloading loop binaries for version: $version")

  val versionDir = binaryDir resolve s"loop-$platform-v$version"
  val location =
    s"https://github.com/lightninglabs/loop/releases/download/v$version/loop-$platform-v$version.$suffix"

  if (Files.exists(versionDir)) {
    logger.debug(
      s"Directory $versionDir already exists, skipping download of loop $version")
  } else {
    val archiveLocation = binaryDir resolve s"$version.$suffix"
    logger.info(s"Downloading loop version $version from location: $location")
    logger.info(s"Placing the file in $archiveLocation")
    val downloadCommand = url(location) #> archiveLocation.toFile
    downloadCommand.!!
    ...
    val expectedHash =
      if (Properties.isLinux)
        "1f1374db976976f4fd761b9d4f7dde2a7803d40e46f75b56b42c71c26fa6a5c8"
      else if (Properties.isMac)
        "217bcba46674daf63c23b78c3f156e7d2b7d9d5a008c39d56898960465c14351"
      else if (Properties.isWin)
        "aa5439566faf0b6cbb1cfd36541273490a02157e2c514b02dc536164711265e4"
      else sys.error(s"Unsupported OS: ${Properties.osName}")
    ...
    if (hash.equalsIgnoreCase(expectedHash)) {
      ...
      logger.error(
        s"Downloaded invalid version of loop, got $hash, expected $expectedHash")
    }
